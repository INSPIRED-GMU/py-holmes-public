...
class TestCircleArea(unittest.TestCase):    # Each test method within this class must start with the word "test".  Each method here constitutes 1 test, and is caled a "test case".
    ...
    def test_file_outside_project_folder(self):
        """Failing test.  Call say_hi() in another project folder, and trigger a failing assert.  All say_hi() does is
        print "Hi!" and return None.
        """
        sys.path.append(os.path.abspath(os.path.dirname(os.path.dirname(__file__)) + "/py-holmes-external-interpreter-for-testing"))
        from my_file import say_hi
        self.assertEqual(3, say_hi())
